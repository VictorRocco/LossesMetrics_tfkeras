...
class bPERCEPTUALl(tf.keras.losses.Loss):
	...
	def _gram_matrix(self, input_tensor):
		...
		num_locations = tf.cast(input_tensor.shape[1] * input_tensor.shape[2] * input_tensor.shape[3], tf.float32)
	...
	def _vgg_style_outputs(self, input):
		...
		#print("_vgg_style_outputs (added dimension) input shape:", input.shape, flush=True)
	...
	def _style_loss(self, y_true, y_pred):
		...
		#print("_style_loss y_true_style_outputs[0] shape:", y_true_style_outputs[0].shape, flush=True)
		y_true_style_outputs = [self._gram_matrix(style) for style in y_true_style_outputs] #out shape.ndims = 3
		#print("_style_loss y_true_style_outputs[0] (after gram_matrix) shape:", y_true_style_outputs[0].shape, flush=True)

		y_pred_style_outputs = self._vgg_style_outputs(y_pred) #out shape.ndims = 4
		y_pred_style_outputs = [self._gram_matrix(style) for style in y_pred_style_outputs] #out shape.ndims = 3
		...
		for true, pred in zip(y_true_style_outputs, y_pred_style_outputs):
			...
			pred_mean, pred_std = self._get_mean_std(pred)

			true_normalized = (true - true_mean) / true_std
			pred_normalized = (pred - pred_mean) / pred_std

			#print("_style_loss true mean std shapes:", true_mean.shape, true_std.shape)
	...
	def _content_loss(self, y_true, y_pred):
		...
		for true, pred in zip(y_true_content_outputs, y_pred_content_outputs):
			...
			# print("_content_loss true mean std shapes:", true_mean.shape, true_std.shape)
